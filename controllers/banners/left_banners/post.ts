...
const post = async (req: any, res: any, refresh_token) => {
    try {
        ...
        if (resBanner) {
            // @ts-ignore
            if (resBanner.banners?.right_banners) bannersData["right_banners"] = resBanner.banners?.right_banners;
            // @ts-ignore
            if (resBanner.banners?.left_banners) bannersData.left_banners.push(...resBanner.banners?.left_banners);
