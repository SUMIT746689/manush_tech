...
export default async function post(req, res, refresh_token) {
  try {
    const { title, amount, last_date, academic_year_id, class_id, school_id, late_fee, months } = req.body;

    if (!title || !amount || !last_date || !academic_year_id || !class_id || !school_id) throw new Error('provide all valid information');
    const data = { title, amount, last_date: new Date(last_date), academic_year_id, class_id, school_id };
