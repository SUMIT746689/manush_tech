...
import { getUsers, insertDataToSentSmsDetails, insertsentSms, sentSmsHandler, updateSentSms } from './postContent/insertDataToSentSmsDetails';
...
async function post(req, res, refresh_token) {
  try {
    ...
    switch (recipient_type) {
      case "GROUP":
        ...
        roles.forEach(async (role) => {
          ...
          sentSmsHandler({ sentSmsUsers: singleGroupUsers, school_id, responseSentSms, smsGatewayRes });
        })
    ...
    sentSmsUsers.length > 0 && sentSmsHandler({ sentSmsUsers, school_id, responseSentSms, smsGatewayRes });
