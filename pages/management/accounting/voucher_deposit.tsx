...
import Results from 'src/content/Management/Voucher/Deposit/Results';
import { useClientFetch } from '@/hooks/useClientFetch';

function ManagementClasses() {
  const [editClass, setEditClass] = useState(null);
  const { data: accounts } = useClientFetch(`/api/account`);
  const { data: voucher } = useClientFetch(`/api/voucher?type=credit`);
  const { data: deposit, reFetchData: reFetchDeposit } = useClientFetch(`/api/transaction?type=credit`);
  ...
  return (
    <>
      ...
      >
        <Grid item xs={12}>
          <Results users={deposit || []} setEditClass={setEditClass} />
        </Grid>
      </Grid>
      <Footer />
    </>
