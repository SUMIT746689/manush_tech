...
export async function getServerSideProps(context: any) {
    ...
    try {
        ...
        for (const property in columns) {
            if (!["user_id", "id", "father_photo", "mother_photo", "student_photo", "guardian_photo", "created_at", "extra_section_id", "school_id", "student_information_id"].includes(columns[property])) without_photo_fields[property] = columns[property];
    ...
    return (
        <>
                ...
                <Grid container justifyContent="space-between" alignItems="center">
                    <Grid item >
                        ...
                        <Typography variant="subtitle2">
                            {t(
                                'All aspects related to the students update can be managed from this page'
                            )}
                        </Typography>
                    </Grid>
                </Grid>
            ...
            <Card sx={{ mx: { sm: 4, xs: 1 }, pr: 1, py: 1, mb: 1, display: "flex", flexWrap: "wrap", columnGap: 0.5 }}>
                <SelectUpdateColumn student_columns={student_columns} selectedUpdateColumns={selectedUpdateColumns} setSelectedUpdateColumns={setSelectedUpdateColumns} />
            </Card>

            <Card sx={{ mx: { sm: 4, xs: 1 }, p: 1, pb: 0, mb: 1, display: "grid", gridTemplateColumns: { sm: "1fr 1fr 1fr", md: "1fr 1fr 1fr auto" }, columnGap: 1 }}>
                {/* select class */}
                ...
                <ButtonWrapper disabled={!selectedSection} handleClick={handleStudentList}>Search</ButtonWrapper>
            ...
            >
                <Grid item xs={12}>
                    <Results
                        students={students?.AllStudents?.map(student => {
...
const SelectUpdateColumn = ({ student_columns, selectedUpdateColumns, setSelectedUpdateColumns }) => {
    ...
    return (
        <>
            <Grid container px={1} fontWeight={500}>Updated Column: *</Grid>
            {
                student_columns.map((key,index) => <>
                    <Grid key={index}> <Checkbox checked={selectedUpdateColumns.includes(key) ? true : false} size="small" sx={{ width: 30, height: 30 }} onClick={(event) => handleSelectStudentUpdateCols(event, key)} /> {key} </Grid>
                </>)
            }
        </>
    )
