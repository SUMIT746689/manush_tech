...
const IncomeReport = () => {
  ...
  const handleSearchClick = async () => {
    let url = `/api/reports/financial_reports/income_report?from_date=${startDate}&to_date=${endDate}`;
    ...
    const data = res?.success ? res.data.map(tran => ({
      ...
      'Amount(Cr.)': tran.voucher.type === 'credit' ? tran.amount : 0
    }))
      :
      [];
