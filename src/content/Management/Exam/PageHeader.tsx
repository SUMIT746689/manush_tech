...
} from '@/components/AutoCompleteWrapper';
    ...
    if (editExam) {
      try {
        ...
        const targetClass = classes?.find(
          (i) => i.id == editExam?.section?.class?.id
        );
        setSections(
          targetClass?.sections?.map((i) => {
            return {
              label: i.name,
              id: i.id
            };
    ...
    try {
      ...
      if (editExam) {
        query = {
          ...
          final_percent: checked
            ? _values.final_percent || editExam?.final_percent
            : 0,
            ...
            successProcess(
              t(
                `${
                  res?.data?.message
                    ? res?.data?.message
                    : 'A new Exam has been updated successfully'
                }`
              )
            );
            getExam();
            ...
            editExam
              ...
              : Yup.object().shape({
                  ...
                  section_id: Yup.number()
                    .min(1)
                    .required(t('The section field is required')),
                  subject_id_list: Yup.array().required(
                ...
                >
                  <Grid container spacing={0}>
                    ...
                    >
                      ...
                      <AutoCompleteWrapper
                        error={Boolean(
                          touched.exam_term_id && errors.exam_term_id
                        )}
                    ...
                    >
                      ...
                      />
                      {/* select section */}
                      {(sections && editExam?.section?.class?.has_section) ||
                      (sections &&
                        selectedClass &&
                        selectedClass?.has_section) ? (
                        <>
                          <Grid
                            item
                            ...
                            >
                              ...
                              <Grid item>
                                <AutoCompleteWrapper
                                  ...
                                  options={rooms}
                                  multiple={true}
                                  value={rooms?.filter((ii) =>
                                    option?.exam_room?.includes(ii.id)
                                  )}
                                  handleChange={(e, v) => {
                                    if (v) {
                                      const temp = [...subject];
                                      temp[index].exam_room = v.map(
                                        (i) => i.id
                                      );
                                      setSubject(temp);
                                      setFieldValue('subject_id_list', temp);
