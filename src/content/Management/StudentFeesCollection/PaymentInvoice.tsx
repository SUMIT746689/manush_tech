...
const PaymentInvoice: FC<PaymentInvoiceType> = ({
  ...
  useEffect(() => {
    ...
    const totalDueValue =
      temp.reduce((prev, curr) => prev + Number(curr.due), 0) || 0;

    setTotalDueAmount(parseInt(totalDueValue));

    const totalAmount =
      temp.reduce((prev, curr) => prev + Number(curr.payableAmount), 0) || 0;

    setTotalFeeamount(totalAmount);

    const totalPaidAmount_ =
      temp.reduce((prev, curr) => prev + (Number(curr.paidAmount) || 0), 0) ||
      0;
  ...
  return (
    ...
    >
      ...
      <Grid
        sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr ', gridGap: 2 }}
        mt={4}
      >
        <Grid>
          <Grid>
        ...
        <Grid>
          ...
          <Grid>
            ...
            Invoice Created:{' '}
            <b>
              {selectedFees[0]?.created_at
                ? dayjs(selectedFees[0]?.created_at).format(
                    'DD/MM/YYYY, h:mm a'
                  )
                : selectedFees[0]?.last_payment_date}
            </b>
          </Grid>
          <Grid>
            {' '}
      ...
      >
        <Grid>
          {' '}
          Account:{' '}
          <b>{selectedFees[0]?.account?.title || selectedFees[0]?.account}</b>
        </Grid>
      ...
      <Grid container pt={4}>
        <TableContainer
          sx={{ border: 1, borderColor: 'gray', borderRadius: 0.5 }}
        >
              ...
              {selectedFees?.map((payment, index) => {
                return (
                  ...
                  >
                    ...
                    <TableCell sx={{ p: 1 }} align="right">
                      {dayjs(payment?.last_payment_date).format(
                        'DD/MM/YYYY, h:mm a'
                      )}
                    </TableCell>
                    <TableCell sx={{ p: 1 }} align="right">
                    ...
                    <TableCell sx={{ p: 1 }} align="right">
                      {payment.prevAmount
                        ? formatNumber(payment?.prevAmount)
                        : 0}
                    </TableCell>
                    <TableCell sx={{ p: 1 }} align="right">
                      {payment.paidAmount
