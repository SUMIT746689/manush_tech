...
const Results: FC<{ students: any[]; refetch: () => void; discount: any[]; idCard: any; fee: any[] }> = ({
  ...
  const handleSelectAllUsers = (event: ChangeEvent<HTMLInputElement>): void => {
    ...
    // updated code start
    setSelectedUsers(event.target.checked ? userIds : []);
  ...
  // updated code
  const selectedAllUsers = isAllChecked;
  ...
  const handleClick = (event: React.MouseEvent<HTMLButtonElement>) => {
    console.log('HandleClick functionality');
  ...
  return (
    <>
      ...
      >
        <Grid item container flexDirection={'column'} sx={{ p: 4 }}>
          ...
          <Grid
            ...
            borderRadius={'8px'}
            sx={{ gridTemplateColumns: { sm: 'repeat(2, minmax(0, 1fr))', md: 'repeat(3, minmax(0, 1fr))' }, gap: { xs: 1, sm: 2 } }}
          >
            {discount?.map((i: any) => (
              <SingleDiscount key={i.id} selectedUser={selectedStudent} singleDiscount={i} />
            ))}
          </Grid>
          <br />
          <Typography fontSize={20} fontWeight={'bold'}>
          ...
          >
            {fee?.map((i: any) => (
              <SingleFee key={i.id} selectedUser={selectedStudent} singleFee={i} />
            ))}
          </Grid>
        </Grid>
      </Dialog>
      ...
      <Grid mt={1}>
        <ButtonWrapper handleClick={handleClick}>Submit</ButtonWrapper>
      </Grid>
    </>
  );
};
...
const SingleFee = ({ singleFee, selectedUser }) => {
  ...
  return (
    <Box
      ...
      key={singleFee.id}
    >
      <Typography sx={{ my: 'auto', textTransform: 'capitalize', fontSize: { xs: 10, md: 15 } }}>{singleFee?.label}</Typography>
...
const SingleDiscount = ({ singleDiscount, selectedUser }) => {
  ...
  return (
    ...
    >
      <Typography sx={{ my: 'auto', textTransform: 'capitalize', fontSize: { xs: 10, md: 15 } }}>{singleDiscount?.label}</Typography>
